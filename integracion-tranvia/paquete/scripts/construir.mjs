...
function pintarJuego(item, i) {
  return `    <article class="recurso recurso--sitio aparece" data-orden="${i}">
      <div class="recurso__superior">
        <div>
          <p class="recurso__autor">Juego</p>
          <h4 class="recurso__titulo">${escapar(item.titulo)}</h4>
          ${item.descripcion ? `<p class="recurso__descripcion">${escapar(item.descripcion)}</p>` : ''}
        </div>
      </div>
      <div class="recurso__acciones">
        <a class="accion accion--primaria" href="${escapar(item.url)}" target="_blank" rel="noopener">Abrir ${FLECHA}</a>
      </div>
    </article>`;
}
...
function pintarRecursos(config, recursos) {
  ...
  const juegos = recursos.juegos || [];
  const otrosSitios = recursos.otrosSitios || [];
  const total = literatura.length + videos.length + otras.length + juegos.length + otrosSitios.length;
...
${pintarSubseccion('juegos', 4, config.juegos || {}, juegos, pintarJuego, { singular: 'juego', plural: 'juegos' })}

${pintarSubseccion('otros-sitios', 5, config.otrosSitios || {}, otrosSitios, pintarOtroSitio, { singular: 'sitio', plural: 'sitios' })}
    </div>
  </section>`;
}
...
async function construir() {
  ...
  console.log(`  ${sesiones.length} sesión(es), ${recursos.literatura.length} lectura(s), ${recursos.videos.length} video(s), ${recursos.otrasClases.length} archivo(s) de otras clases, ${recursos.juegos.length} juego(s), ${recursos.otrosSitios.length} sitio(s) externo(s).`);

...
export default async function handler(req,res){
  ...
  try{
    ...
    for(const row of current){
      ...
      const t=tests[summary.testId] ||= {testId:summary.testId,participants:0,completed:0,active:0,abandoned:0,totalAttempts:0,questions:{}};
      t.participants++;
      t[summary.status]++;
      t.totalAttempts+=summary.attempts;
      const questions=row.payload?.questions||{};
      for(const [questionId,st] of Object.entries(questions)){
        const q=t.questions[questionId] ||= {questionId,engaged:0,resolved:0,revealed:0,firstAttempts:0,firstAttemptCorrect:0,totalAttempts:0,activeTimes:[],firstChoiceCounts:{},allChoiceCounts:{}};
        ...
        if(engaged) q.engaged++;
        if(completed(st)) q.resolved++;
        if(st?.everRevealed) q.revealed++;
        q.totalAttempts+=attempts.length;
        if(Number.isFinite(Number(st?.activeMs))) q.activeTimes.push(Number(st.activeMs));
        if(attempts[0]){
          q.firstAttempts++;
          if(attempts[0].correct) q.firstAttemptCorrect++;
          if(attempts[0].optionId) q.firstChoiceCounts[attempts[0].optionId]=(q.firstChoiceCounts[attempts[0].optionId]||0)+1;
        }
        for(const attempt of attempts){
          if(attempt?.optionId) q.allChoiceCounts[attempt.optionId]=(q.allChoiceCounts[attempt.optionId]||0)+1;
        }
      }
    }
    ...
    return reply(res,200,{generatedAt:new Date().toISOString(),tests,runs,totalStoredRuns:rows.length});
